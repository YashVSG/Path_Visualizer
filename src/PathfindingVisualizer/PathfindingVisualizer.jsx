...
export default class PathfindingVisualizer extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      topMessage: "Dijkstra Algorithm / Bellman_Ford",
      weight: 1,
      changeWeight: false,
  ...
  handleMouseDown(row, col) {
    if (this.state.topMessage !== "Dijkstra Algorithm / Bellman_Ford") return;
  ...
  visualizeDijkstra() {
    this.setState({ topMessage: "Creator : Yash" });
  ...
  visualizeBellman() {
    ...
    const startNode = grid[START_NODE_ROW][START_NODE_COL];
    const finishNode = grid[FINISH_NODE_ROW][FINISH_NODE_COL];
    const {nodesInShortestPathOrder ,visitedNodesInOrder} = bellmanFord(
      grid,
      startNode,
      finishNode
    );
  ...
  animateDijkstra(visitedNodesInOrder, nodesInShortestPathOrder) {
    for (let i = 0; i <= visitedNodesInOrder.length; i++) {
      ...
      if(node === this.state.grid[START_NODE_ROW][START_NODE_COL]) continue;
      if(node === this.state.grid[FINISH_NODE_ROW][FINISH_NODE_COL]) continue;     
  ...
  animateShortestPath(nodesInShortestPathOrder) {
    let timeTaken = nodesInShortestPathOrder.length - 1;;
  ...
  render() {
    ...
    let button_task = (
      ...
      <br/>
      <p className="btn" onClick={() => this.visualizeBellman()}>
        Start Bellman_Ford Algorithm
      </p>
      </>
    );
    ...
    } else if (topMessage === "Creator : Yash") {
